...
class Netty4TcpTransportServerActor(configuration: Netty4Configuration) extends Actor with ActorLogging {
  ...
  private def associationEventListenerRegistered(serverChannel: Channel, listener: AssociationEventListener): Actor.Receive = {
    case ChannelActive(channel) =>
      ...
      try {
        //FIXME defence the virus?!
        log.info("remote socket address :{}", channel.remoteAddress())
        val remoteAddress = Netty4TcpTransport.inetAddressToActorAddress(channel.remoteAddress().asInstanceOf[InetSocketAddress])
        log.info("remote actor address :{}", remoteAddress)
        log.debug("remote client connected in,remote address :{}", remoteAddress)
        val associator = context.actorOf(Props.create(
          classOf[Netty4TcpTransportAssociator],
          remoteAddress,
        ...
        case NonFatal(e) =>
          log.error(e, "socket exception ,maybe cased by https://github.com/netty/netty/issues/3328")
          log.error("socket is error,so we have to close the channel for now")
          for (future <- Try { channel.close() }.toOption) {
            log.error("close the socket successfully")
          }
      }
...
case class ChannelBound(channel: Channel) extends NoSerializationVerificationNeeded

case class AssociationEventListenerRegisterSuccess(listener: AssociationEventListener) extends NoSerializationVerificationNeeded
case class AssociationEventListenerRegisterFailure(exception: Throwable) extends NoSerializationVerificationNeeded
